...
int main()
{
	...
	//ICameraSceneNode* miniCamera = smgr->addCameraSceneNode(camera, vector3df(0.0f, 5.0f, 0.0f), posCam, -1, false);
	...
	while (device->run() && isRunning)
	{
		...
		vector3df posCam = camera->getPosition();
		...
		if (GetKeyState('M') & 0x8000 ) 
		{
			...
			ICameraSceneNode* miniCamera = smgr->addCameraSceneNode(camera, vector3df(0.0f, 5.0f, 0.0f), posCam, -1, true);
